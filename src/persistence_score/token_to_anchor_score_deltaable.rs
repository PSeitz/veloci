...
impl TokenToAnchorScoreVintDelta {
    ...
    fn get_scores_disk(&self, id: u32) -> Option<Vec<AnchorScore>> {
        ...
        let start_pos_mmap = unsafe { MmapOptions::new().map(&start_pos_file).unwrap() };

        let data_mmap = unsafe { MmapOptions::new().map(&data_file).unwrap() };
    ...
    pub fn add_values_to_disk(
        free_blocks: &mut Vec<FreeBlock>,
        id: u32,
        mut add_data: &mut Vec<u32>,
        start_pos_mmap: &mut MmapMut,
        data_mmap: &mut MmapMut,
        append_data: &mut Vec<(usize, Vec<u8>)>,
    ) {
        ...
        if start_pos_mmap.len() < required_size || get_u32_from_mmap(&start_pos_mmap, pos) == NO_DATA {
            ...
            if old_size != new_size {
                ...
                if let Some(free_block_pos) = Self::get_free_block(free_blocks, new_data.len() as u32) {
                    ...
                    // append data_mmap to end of file
                    append_data.push((pos, new_data));
                }
            } else {
    ...
    pub fn flush_to_disk(&mut self) -> Result<(), io::Error> {
        let start_pos_file = OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
            .open(self.start_pos_file_path.to_string())
            .unwrap();
        let data_file = OpenOptions::new()
            .read(true)
            .write(true)
            .create(true)
            .open(self.data_file_path.to_string())
            .unwrap();
...
fn test_token_to_anchor_score_delta() {
    ...
    assert_eq!(
        yeps.get_scores(1),
        Some(vec![AnchorScore::new(1, f16::from_f32(1.0)), AnchorScore::new(2, f16::from_f32(1.0))])
    );
    assert_eq!(yeps.get_scores(2), None);
    ...
    assert_eq!(
        yeps.get_scores(3),
        Some(vec![AnchorScore::new(2, f16::from_f32(1.0)), AnchorScore::new(5, f16::from_f32(1.0))])
    );;
