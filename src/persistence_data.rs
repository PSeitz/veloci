...
impl<T: IndexIdToParentData> IndexIdToParent for IndexIdToMultipleParent<T> {
    ...
    fn get_values(&self, id: u64) -> Option<Vec<T>> {
        self.data.get(id as usize).map(|el| el.iter().map(|el| NumCast::from(*el).unwrap()).collect())
    }
...
impl<T: IndexIdToParentData> IndexIdToParent for IndexIdToMultipleParentCompressedMaydaDIRECT<T> {
    ...
    default fn get_values(&self, id: u64) -> Option<Vec<T>> {
        self.data.get(id as usize).map(|el| {
            el.decode().iter().map(|el| NumCast::from(*el).unwrap()).collect()
        })
    }
    fn get_keys(&self) -> Vec<T> {
        (NumCast::from(0).unwrap()..NumCast::from(self.data.len()).unwrap()).collect()
...
#[inline(always)]
fn get_values_indirect_generic<T, K, M>(id: u64, size:u64, start_and_end: &T, data: &K) -> Option<Vec<M>> where
...
impl<T: IndexIdToParentData> IndexIdToOneParent<T> {
    pub fn new(data: &IndexIdToParent<Output=T>) -> IndexIdToOneParent<T> {
        ...
        let data = data.iter().map(|el| if el.len() > 0 { NumCast::from(el[0]).unwrap() } else { NumCast::from(NOT_FOUND).unwrap() }).collect();
...
impl<T: IndexIdToParentData> IndexIdToParent for IndexIdToOneParent<T> {
    ...
    fn get_value(&self, id: u64) -> Option<T> {
        ...
        match val {
            Some(val) => if val.to_u32().unwrap() == NOT_FOUND {
                None
            } else {
                Some(*val)
            },
...
impl<T: IndexIdToParentData> IndexIdToOneParentMayda<T> {
    ...
    pub fn new(data: &IndexIdToParent<Output=T>) -> IndexIdToOneParentMayda<T> {
        ...
        IndexIdToOneParentMayda { size: yep.data.len(), data: to_uniform(&yep.data) }
    }
    #[allow(dead_code)]
    pub fn from_vec(data: &Vec<T>) -> IndexIdToOneParentMayda<T> {
        IndexIdToOneParentMayda { size: data.len(), data: to_uniform(&data) }
    }
}
...
impl<T: IndexIdToParentData> SingleArrayFileReader<T> {
    pub fn new(data_file: fs::File, data_metadata:fs::Metadata) -> Self {
        SingleArrayFileReader {data_file: Mutex::new(data_file), data_metadata: Mutex::new(data_metadata), ok:PhantomData  }
    }
...
impl<T: IndexIdToParentData>  IndexIdToParent for SingleArrayFileReader<T> {
    type Output = T;
    default fn get_value(&self, _find: u64) -> Option<T> {unimplemented!() }
    default fn get_values(&self, _find: u64) -> Option<Vec<T>> {unimplemented!() }
    fn get_keys(&self) -> Vec<T> {
        (NumCast::from(0).unwrap()..NumCast::from(self.get_size()).unwrap()).collect()
    }
}
...
impl IndexIdToParent for SingleArrayFileReader<u64> {
    fn get_value(&self, find: u64) -> Option<u64> {
        let block_size = 8;
        let size = self.data_metadata.lock().len() as usize / block_size;
        if find >= size as u64 {return None;}
        let data_bytes = load_bytes(&*self.data_file.lock(), find as u64 * block_size as u64, block_size);
...
fn to_indirect_arrays<T: Integer + Clone + NumCast + mayda::utility::Bits + Copy, K:IndexIdToParentData>(store: &IndexIdToParent<Output=K>, cache_size:usize) -> (Vec<T>, Vec<T>) {
    let mut data = vec![];
    let mut valids = store.get_keys();
    valids.dedup();
    if valids.len() == 0 {
...
pub fn id_to_parent_to_array_of_array_mayda_indirect_one<T: Integer + Clone + NumCast + mayda::utility::Bits + Copy, K:IndexIdToParentData>(store: &IndexIdToParent<Output=K>) -> (usize, mayda::Monotone<T>, mayda::Uniform<T>) { //start, end, data
...
pub fn id_to_parent_to_array_of_array_mayda_indirect_one_reuse_existing<T: Integer + Clone + NumCast + mayda::utility::Bits + Copy, K:IndexIdToParentData>(store: &IndexIdToParent<Output=K>) -> (usize, mayda::Uniform<T>, mayda::Uniform<T>) { //start, end, data
...
pub fn valid_pair_to_parallel_arrays<T: IndexIdToParentData>(tuples: &mut Vec<create::ValIdPair>) -> ParallelArrays<T> {
    ...
    let valids =         tuples.iter().map(|ref el| NumCast::from(el.valid).unwrap()).collect::<Vec<_>>();
    let parent_val_ids = tuples.iter().map(|ref el| NumCast::from(el.parent_val_id).unwrap()).collect::<Vec<_>>();
...
pub fn boost_pair_to_parallel_arrays<T: IndexIdToParentData>(tuples: &mut Vec<create::ValIdToValue>) -> ParallelArrays<T> {
    tuples.sort_by(|a, b| a.valid.partial_cmp(&b.valid).unwrap_or(Ordering::Equal));
    let valids = tuples.iter().map(|ref el| NumCast::from(el.valid).unwrap()).collect::<Vec<_>>();
    let values = tuples.iter().map(|ref el| NumCast::from(el.value).unwrap()).collect::<Vec<_>>();
...
mod test {
    ...
    mod test_direct_1_to_1 {
        ...
        fn test_single_file_array() {
            ...
            File::create("test_single_file_array/data").unwrap().write_all(&vec_to_bytes_u64(&store.data)).unwrap();
