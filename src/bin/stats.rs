    ...
    for path in paths {
        ...
        for id in 0..token_to_anchor_score.get_max_id() {
            ...
            if let Some(text_id_score) = token_to_anchor_score.get_scores(id as u32) {
                ...
                let mut map_scores = text_id_score.iter().map(|el|el.score.to_f32() as u32).fold(FnvHashMap::default(), |mut m, c| { *m.entry(c).or_insert(0) += 1; m });

                total_comp += increases.iter()
                    .map(|&inc| {if inc < 128 { 1 } else if inc < 16000{ 2 } else if inc < 2_000_000 { 3 } else { 4 } }).sum::<usize>();
                ...
                let sum_comp_enc_score = increases.iter()
                    .map(|&inc| {if inc < 64 { 1 } else if inc < 8000{ 2 } else if inc < 1_000_000 { 3 } else { 4 } }).sum::<usize>();
                ...
                if map_scores.len() >= 3 {
                    ...
                    total_comp_enc_score_three += increases.iter()
                    .map(|&inc| {if inc < 32 { 1 } else if inc < 4000{ 2 } else if inc < 500_000 { 3 } else { 4 } }).sum::<usize>();
                    total_comp_enc_score_three -= occurences;

                }else{ //not enough values, take single encoded
