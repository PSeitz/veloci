...
pub fn get_hits_in_field_one_term(persistence:&Persistence, options: &RequestSearchPart) -> Result<SearchFieldResult, SearchError> {
    ...
    trace!("Will Check starts_with {:?}", options.starts_with);
    {
        let teh_callback = |line: &str, line_pos: u32| {
            ...
            let distance = if options.levenshtein_distance.unwrap_or(0) != 0 { Some(distance(&options.terms[0], line))} else { None }; //TODO: find term for multitoken
            let mut score = if distance.is_some() {get_default_score2(distance.unwrap(), prefix_matches)}
                else {get_default_score(&options.terms[0], line, prefix_matches)};
